...
const ZIP3GEO = (function() {
  ...
  function getCoords(zip3) {
    ...
    // Deterministic pseudo-random scatter using the zip3 value as seed
    // This avoids S-patterns by hashing the zip into a 2D offset
    function hash(n) {
      let h = n * 2654435761 >>> 0; // Knuth multiplicative hash
      return (h & 0xFFFF) / 0xFFFF; // 0.0 to 1.0
    }
    const hLat = hash(z * 7 + 31) - 0.5;  // -0.5 to 0.5
    const hLng = hash(z * 13 + 53) - 0.5;

    const latOffset = hLat * spread[0];
    const lngOffset = hLng * spread[1];
